...
fmapSpunHalf = np.array((np.zeros((4,m+z-2,n+z-2), dtype=np.float)))
fmapFlipHalf = np.array((np.zeros((4,m+z-2,n+z-2), dtype=np.float)))
...
def calcForceMap(gforms, fmapfull, fmaphalf):
	forcemap = fmapfull
	forms = np.shape(gforms)[0]
	for form in xrange(forms):
		row = np.shape(gforms[form])[0]
		col = np.shape(gforms[form])[1]
		xmap = np.shape(forcemap[form])[0]
		ymap = np.shape(forcemap[form])[1]
		for i in xrange(xmap):
			for j in xrange(ymap):
				forcemap[form][i,j] = np.sum(gforms[form] *
				randomHost[i*rez:i*rez+row,j*rez:j*rez+col])
		for i in xrange((hostsize[0]-1)/rez-rez):
			for j in xrange((hostsize[1]-1)/rez-rez):
				upleft = forcemap[form][i,j]/2
				downleft = forcemap[form][i+1,j]/2
				upright = forcemap[form][i,j+1]/2
				downright = forcemap[form][i+1,j+1]/2
				if i % 2 == 0 and j % 2 == 1:
					fmaphalf[form][i,j] = upleft + upright
				if i % 2 == 1 and j % 2 == 0:
					fmaphalf[form][i,j] = upleft + downleft
				if i % 2 == 1 and j % 2 == 1:
					fmaphalf[form][i,j] = upleft + downleft + upright + downright
	return
...
calcForceMap(spunGuestsFull, fmapSpunFull, fmapSpunHalf)
calcForceMap(flipGuestsFull, fmapFlipFull, fmapFlipHalf)
...
findExtrema(fmapSpunHalf,lminSH,lmaxSH,gminSH,gmaxSH,'the half-shifted')
findExtrema(fmapFlipHalf,lminFH,lmaxFH,gminFH,gmaxFH,'the half-shifted, flipped')
