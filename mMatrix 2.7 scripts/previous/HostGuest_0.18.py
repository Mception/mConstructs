...
hostsize = ((m+z*2-1)*rez+1, (n+z*2-1)*rez+1)
randomHost = np.zeros(hostsize)
hostRegion = np.array(np.random.uniform(-1, 1, (m,n)))
for i in xrange(0, m*rez-1, rez):
    for j in xrange(0, n*rez, rez):
        randomHost[i+z*2,j+z*2] = hostRegion[i/rez,j/rez]
...
mforceMap= np.array((np.zeros((4,z*rez-1,z*rez-1), dtype=np.float)))
#if x<>y:
