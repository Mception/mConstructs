...
Updated on Thu Dec 04 22:06:00 2017
@author: MJL
"""
...
rez = int(raw_input("Enter resolution of chemical space (1 = low; 4 = high): "))
#resolution value is made even for more direct host-guest calculations

if rez <1 or rez > 4:
	rez = int(raw_input("Re-enter resolution between 1 = low and 4 = high: "))
if rez <1 or rez > 4:
	print 'Try running script again.'
	raise SystemExit

rez = (rez)*2 # resolution between main charges of 2, 4, 6, or 8 
...
print "\nThis may take around {} min(s) to complete an iPad A7 processor.".format(time/120000+(m+n+2*z)*rez/600)
...
if x>y:
	...
	guestSquared = guestRegion
else:
	guestSquared = np.r_[guestRegion, [np.zeros(y)]]

guestSpun45 = np.array(np.zeros((z*2-1,z*2-1)))
for i in xrange(z):
	for j in xrange(z):
		guestSpun45[(i+j),(j-i)+(z-1)] = guestSquared[i,j]
...
def calcForceMap(gforms, fmapfull):
	...
	for form in xrange(forms): #this calculates the full (ontop) interactions
		for i in xrange(0,xmap,rez):
			for j in xrange(0,ymap,rez):
				forcemap[form][i,j] = np.sum(gforms[form] *
				randomHost[i:i+gsize,j:j+gsize])
		for i in xrange(xmap): #this inserts the half-shifted (inbetween) values
