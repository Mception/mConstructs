...
Updated on Wed Dec 15 00:41:00 2017
...
r = int(raw_input("Enter no. of row-atom-charges for binding region (>=2): "))
c = int(raw_input("Enter no. of col-atom-charges for binding region (>=2): "))
m = int(raw_input("Enter longest no. of linear atoms for molecule (>=2): "))
...
time = float(r*c*2*md)
print '\nThis may take around {} min(s) to complete an iPad A7 processor.'.format('{0:.2f}'.format(time/120000+(r+c+2*md)/300)
...
for i in xrange(0,z,3): # generates a random molecule to test routines
	for j in xrange(0,z,3):
		#mMatrix[i:i+3,j:j+3] = atomMatrix
		mMatrix[i,j] = i/3*md+j/3+1
		mMatrix[i+1,j+1] = np.around(np.random.uniform(-1, 1), decimals=1) 
		mdMatrix[i/3][j/3] = mMatrix[i:i+3,j:j+3]
		if i%2==1 or j%2==1:
...
for i in xrange(0, r*rez, rez):
	for j in xrange(0, c*rez, rez):
		bRegion[i+m*rez-rez,j+m*rez-rez] = bCharges[i/rez,j/rez]
...
def mdConvert(cpd,mdcpd,md): # reduces md-forms to m-forms, and vice versa
	z = np.shape(cpd)[0]
	if md == 0:
		for i in xrange(0,z,3):
			for j in xrange(0,z,3):
				mdcpd[i/3][j/3] = cpd[i:i+3,j:j+3]
	if md == 1:
		for i in xrange(0,z,3):
			for j in xrange(0,z,3):
				cpd[i:i+3,j:j+3] = mdcpd[i/3][j/3]
	return 
...
def calcForceMap(mcforms, fmap): #45 degree forms will be treated separately
	...
	for form in xrange(0,forms,2): #calculate the full (ontop) interactions
		for i in xrange(0,xmap,rez):
			for j in xrange(0,ymap,rez):
				forcemap[form][i,j] = np.sum(mcforms[form] *
				bRegion[i:i+size,j:j+size])
		for i in xrange(xmap): #insert the half-shifted (inbetween) values
			for j in xrange(ymap):
				if i%rez==0 and j%rez==1:
					forcemap[form][i,j] = (forcemap[form][i,j-1] +
					forcemap[form][i,j+1])/2
				if i%rez==1 and j%rez==0:
					forcemap[form][i,j] = (forcemap[form][i-1,j] +
					forcemap[form][i+1,j])/2
				if i%rez==1 and j%rez==1:
					forcemap[form][i,j] = (forcemap[form][i-1,j-1] +
					forcemap[form][i+1,j-1] +
					forcemap[form][i-1,j+1] +
					forcemap[form][i+1,j+1])/2
	return
...
print
print 'Molecular binding charges, region, surface, or entity:'
#print bRegion
print bRegion[md-1:2*r+md-1:2,md-1:2*c+md-1:2]

print
print 'Molecules rotated by 0, 45, 90, 135, 180, 225, 270, 315:'
#print spunMcs
print spunMcs[...,::2,::2]

print
print 'Flipped molecules rotated by 0, 45, 90, 135, 180, 225, 270, 315:'
#print flipMcs
print flipMcs[...,::2,::2]
